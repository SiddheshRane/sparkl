 ...
 * load on the database and network reduces. This is the currently employed method. For pros and cons see
 * {@link TagArticlesFromDatabase}.
 *
 ...
 */
public class TagNEFromFile implements Serializable {
    ...
    final static Property TYPE_PROPERTY = ResourceFactory.createProperty("http://www.w3.org/1999/02/22-rdf-syntax-ns#type");
    ...
    public TagNEFromFile(String file, int partitions, final String... fusekiServer) {
        this.file = file;
    ...
    public void extractTaggedSentences() {
        ...
        final String path = "ne-" + partitions + '-'
        ...
        spark.read()
            ...
            .mapPartitions((Iterator<String> it) -> {
                ...
                totalEntities.add(count);
                return Collections.singleton(sb.toString()).iterator();
            }, Encoders.STRING())
            //            .map(this::parseTurtle, MODEL_ENCODER)
            //            .map(this::getProperties, MODEL_ENCODER)
            //            .map(this::populateLabelsAndTypes, MODEL_ENCODER)
            .map(s -> {
                Model m = parseTurtle(s);
                m = getProperties(m);
                m = populateLabelsAndTypes(m);
                return m;
            }, MODEL_ENCODER)
            .flatMap(this::extractNESentences, RELATION_ENCODER)
     ...
     */
    public Model populateLabelsAndTypes(Model model) {
       
        // get labels for all objects
        StringBuilder qlabels = new StringBuilder("CONSTRUCT { ?obj rdfs:label ?label; rdf:type ?type } WHERE { VALUES ?obj { ");
        model.listObjects()
            .filterKeep(RDFNode::isURIResource)
            .mapWith(RDFNode::asResource)
            .mapWith(Resource::getURI)
            .forEachRemaining(t -> qlabels.append(" <").append(t).append(">"));
        qlabels.append("} \n  ?obj rdfs:label ?label; rdf:type ?type . }");
    ...
    public Iterator<RelationSentence> extractNESentences(Model model) {
        List<Resource> subjects = model.listSubjectsWithProperty(ABSTRACT_PROPERTY).toList();
        // prepare label cloud, mapping an Resource which is a URI, to its string
        // representation
        HashMap<Resource, String> labelcloud = new HashMap<>();
        for (Resource person : subjects) {
            ...
            person.listProperties().filterDrop(abs::equals)
                .filterDrop(s-> s.getObject().isLiteral())
                .forEachRemaining(s -> {
                    Resource object = s.getObject().asResource();
                    Literal label =  object.getProperty(LABEL_PROPERTY).getLiteral();
        ...
        HashMap<TokenSequencePattern, Resource> pattern2node = new HashMap<>();
        for (Map.Entry<Resource, String> entry : labelcloud.entrySet()) {
        ...
        for (Resource subject : subjects) {
            ...
            HashMap<Resource, Set<Resource>> node2type = new HashMap();
            ...
            for (CoreMap sentence : sentences) {
                ...
                for (SequenceMatchResult<CoreMap> match : matches) {
                    ...
                    Resource node = pattern2node.get(pattern);
                    ...
                    for (Resource p : model.listObjectsOfProperty(node, TYPE_PROPERTY).mapWith(RDFNode::asResource).toSet()) {
